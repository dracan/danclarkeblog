...
namespace DanClarkeBlog.Core.Helpers
{
    public class BlogPostMarkdownRenderer : IBlogPostRenderer
    {
        ...
        private static string ConvertMarkdownToHtml(string markdown)
        {
            ...
            return Markdig.Markdown.ToHtml(Regex.Replace(markdown, @"(!\[.*?\]\()(.*?)\)", x => $"{x.Groups[1].Value}https://danclarkeblog.blob.core.windows.net/{x.Groups[2].Value})")); //(todo) Don't hardcode this
        }
    }
}